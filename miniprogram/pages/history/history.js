...
Page({
  ...
  clearAll() {
    wx.showModal({ title:'确认清空', content:'确定清空所有下载历史？',
      success: async res => {
        if (res.confirm) {
          try { await clearDownloadHistoryApi(); this.setData({list:[],total:0}); wx.showToast({title:'已清空',icon:'success'}) }
  ...
  async deleteItem(e) {
    ...
    try { await deleteDownloadHistory(id); this.setData({list:this.data.list.filter(i=>i.id!==id)}); wx.showToast({title:'已删除',icon:'success'}) }
