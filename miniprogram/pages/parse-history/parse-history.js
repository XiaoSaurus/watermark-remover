...
Page({
  ...
  clearAll() {
    wx.showModal({
      title: '确认清空', content: '确定清空所有解析历史？',
      success: async res => {
        if (res.confirm) {
          try {
            await clearParseHistoryApi()
          ...
          wx.saveVideoToPhotosAlbum({
            ...
            fail(err) {
              ...
              if (err.errMsg && err.errMsg.includes('auth deny')) {
                wx.showModal({ title: '需要相册权限', content: '请授权访问相册',
                  confirmText: '去授权', success(r) { if (r.confirm) wx.openSetting() } })
              }
            }
          })
        } else {
  ...
  _resetDownload() {
    setTimeout(() => {
      this.setData({ downloadingKey: null, downloadProgress: 0, downloadStatus: '' })
    }, 1000)
  },
