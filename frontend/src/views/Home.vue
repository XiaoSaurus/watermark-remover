        ...
        <div v-if="result" class="result-card">
          ...
          <div v-if="downloadStatus" class="progress-wrap">
            <div class="progress-header">
              <span class="progress-label">{{ downloadStatus==='saving'?'⏳ 保存中...':'⬇️ 下载中' }}</span>
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      await saveParse({
        ...
        shareUrl: inputUrl.value.trim(),
        videoUrls: res.data.videoUrls
      })
    } else {
      ElMessage.error(res.message || '解析失败')
    }
  } catch { ElMessage.error('请求失败，请检查后端服务是否启动') }
...
function downloadVideo(url, quality, idx) {
  ...
  const base = { platform:result.value?.platform, title:result.value?.title,
                 cover:result.value?.cover, quality, videoUrl:url }
...
function resetDownload() {
  setTimeout(() => { downloadingIndex.value=-1; downloadProgress.value=0; downloadStatus.value='' }, 1000)
}
...
.input-card{background:#fff;border-radius:16px;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.15)}
...
.result-card{background:#fff;border-radius:16px;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.15);margin-top:20px}
...
.guide-card ol{padding-left:20px;line-height:2;opacity:.9}
